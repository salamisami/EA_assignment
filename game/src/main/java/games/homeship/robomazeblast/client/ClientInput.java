...
import games.homeship.robomazeblast.client.gui.PlaygroundPanel;
import games.homeship.robomazeblast.client.gui.LoadingPanel;
import games.homeship.robomazeblast.client.io.Resource;
import games.homeship.robomazeblast.net.Event;
import games.homeship.robomazeblast.net.EventReceiverBase;
...
public class ClientInput extends EventReceiverBase implements
    ...
    public void continueLogin(Event event) {
        ...
        continueLogin(challenge);
    }

    // For local games
    private void continueLogin(long challenge) {
        String nickname = "_localuser_";
        String password = "";
        long hash = CHAP.createChecksum(challenge, password);

        // The client requests a login
        ClientThread.getInstance().getServer().login2(new Event(new Object[] {
                nickname, hash }));
    }
    ...
    public void gameStarted(Event event) {
        ...
        MainFrame.getInstance().setContentPane(
                new PlaygroundPanel(Playground.DEFAULT_WIDTH, Playground.DEFAULT_HEIGHT,
                        spectStatus, ClientThread.getInstance().getServer()));
    ...
    public void gameStopped(Event event) {
        ...
        switch (condition) {
            ...
            default -> this.gameStoppedMessage = null;
        }
    ...
    public void gameJoined(Event event) {
        ...
        MainFrame.getInstance().setContentPane(new LoadingPanel(gameName));
        MainFrame.getInstance().setVisible(true);
    }
    ...
    public void loggedIn(Event event) {
        ...
        ClientThread.getInstance().getServer().createGame(session, "_localgame_");
        ClientThread.getInstance().getServer().joinGame(session, "_localgame_");
