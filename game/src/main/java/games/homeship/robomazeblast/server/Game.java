...
public class Game implements Serializable {
    ...
    private static final long   serialVersionUID         = -3038837010162129210L;

    private Session             creator                  = null;
    private String              gameName                 = null;
    private final List<Session> playerSessions           = new ArrayList<>();
    private final List<Player>  players                  = new ArrayList<>();
    private final List<Session> spectSessions            = new ArrayList<>();
    ...
    public boolean addPlayer(Player player) {
        ...
        players.add(player);
        player.setGame(this);
        System.out.println("Player" + player.getID() + "added to Playground ("
                + player.getNickname() + ")");
        return true;
    }
    ...
    /**
     * Forces the server to update the Playground, e.g. when an AI player has
     * moved.
     */
    public void forcePlaygroundUpdate() {
        this.playgroundUpdateRequired = true;

        Server.getInstance().notifyServerLoop();
    }
    ...
    public void removePlayer(int x, int y, Player player) {
        if (this.playground.getElement(x, y).equals(player)) // Removes only the
                                                             // selected player
    ...
    public boolean movePlayer(Player player, int dx, int dy) {
        ...
        if (el == null) // oder Extra
        {
            ...
            this.playground.setElement(player.getX(), player.getY(),
                    player.getID(), null);
