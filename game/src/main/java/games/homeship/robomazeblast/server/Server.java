...
import games.homeship.robomazeblast.net.LogoutReason;
import games.homeship.robomazeblast.server.api.GameInfo;
import games.homeship.robomazeblast.server.api.Session;
import games.homeship.robomazeblast.util.CHAP;
...
/**
 * Main server class. This huge singleton class contains most of the server's
 * logic.
 *
...
public class Server {
     ...
     */
    private final Map<Session, ServerListenerInterface> clients      = new ConcurrentHashMap<>();
    /**
     ...
     */
    private final Map<Session, Game>                    playerToGame = new ConcurrentHashMap<>();
    /**
     * Stores the Session => IP-Address relation. This Map is instance of
     * ConcurrentHashMap and therefor thread-safe.
     */
    private final Map<Session, String>                  playerToIP   = new ConcurrentHashMap<>();
    /**
     ...
     */
    private final BlockingQueue<List<Object>>           explosions   = new LinkedBlockingQueue<>(
                                                                             25);
    ...
    /**
     * Awakes the server loop thread when a game instance requested an update.
     */
    public void notifyServerLoop() {
        assert serverLoop != null;

        synchronized(serverLoop) {
            if (!serverLoop.isAlive()) {
                serverLoop.start();
            } else {
                serverLoop.enablePendingUpdate();
                serverLoop.notify();
            }
        }
    }
    ...
    public void stopGame(Game game) {
        ...
        for (Session sess : game.getPlayerSessions()) {
            ...
            if (sli != null) {
                // Condition "1" means stopped by server
    ...
    public void move(Session session, int x, int y) {
        ...
        Game game = playerToGame.get(session);
        if (game == null) {
            System.out.println("Cannot move because game is null");
            return;
        }
    ...
    public boolean continueGame(Session session, String gameName) {
        if (!clients.containsKey(session)) {
            return false;
        }
    ...
    public boolean pauseGame(Session session, String gameName) {
        if (!clients.containsKey(session)) {
            return false;
        }
